<template>
  <el-container>
  	<el-header height='80px'>
  		<div class="header">
  			<div class="nav-left">
  				<img src="//hcdn1.luffycity.com/static/frontend/activity/head-logo_1564141048.3435316.svg" alt="路飞logo">
  			</div>

  			<div class="nav-center">
  				<ul>
  					<li v-for='(nav,i) in navlinks' :key='nav.id' >
  						<router-link :to='{name:nav.name}'>{{ nav.title }}</router-link><span v-if='i===2'>New</span>
  					</li>
  				</ul>
  			</div>
  			...
  			<div class="nav-right" v-if='userInfo.access_token' @mouseenter='enterHandler' @mouseleave='leaveHandler'>
  				...
  				<ul class="my_account" v-show='isShow'>
  					...
  					<li @click='shop_cart'>购物车<span class="count">({{userInfo.shop_cart_num}})</span><i>></i></li>
  					<li>退出<i>></i></li>
  				</ul>
  			</div>
