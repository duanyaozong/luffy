		        ...
		        <el-table-column prop="time" label="有效期" width="212">
		          <template slot-scope="scope">
		            ...
		            <select v-model = 'scope.row.validPeriodId' @change = 'changeHandler(scope.row)'>
		              <option v-for = '(item,index) in scope.row.validPeriodChoices' :key = 'index' :value = 'item.validPeriodId'>有效期{{item.validPeriod}}</option>
		            </select>
		          </template>
		        </el-table-column>
	...
	methods:{
		...
		deleteRow(index, row){
			let params = {
        		removeShopFromCart:[
                	...
                	valid_period_id:row.validPeriodId
           			}
        		]
    		}
    		this.$http.shopCartRemove(params)
    		.then(res=>{
    			if(res.error_no === 0){
    				this.$message('删除课程成功');
		...
		changeHandler(row){
	        ...
	        .then(res=>{
	        	console.log(res)
	        	if(res.error_no === 0){
	        		//更改有效期套餐，后端返回相应的课程套餐，将原来的 课程列表的数据替换掉
